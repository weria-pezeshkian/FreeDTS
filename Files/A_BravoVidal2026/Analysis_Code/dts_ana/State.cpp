...
bool State::ReadAnalysisInputFile(std::string file)
{
    ...
    while (input >> firstword) {
        ...
        if(firstword == AnalysisVariables::GetFluctuationSpectrumName()) { // "Integrator_Type"
            int nx,ny,analysis_type;
            input >> str >> type >> nx >> ny >> analysis_type;
            if(type =="on"){
                m_pFluctuationSpectrum = new FluctuationSpectrum(this,nx,ny,analysis_type);    
            }
            m_pAnalysisVariables->SetFluctationSpectrumActive();
            m_pAnalysisVariables->SetFluctuationSpectrumExtended(analysis_type);
            getline(input,rest);
        }
        else if(firstword == AnalysisVariables::GetInclusionTrackingName()) { // "Integrator_Type"
            int nx,ny,analysis_type;
            input >> str >> type >> nx >> ny >> analysis_type;
            if(type =="on"){
                m_pInclusionTracking = new InclusionTracking(this);
            }
            m_pAnalysisVariables->SetInclusionTrackingActive();
            getline(input,rest);
        }
        ...
        else if(firstword==AnalysisVariables::GetRDFName()){
            double dr;
            input>>str>>type>>dr;
            m_dr = dr;
            std::cout<<"RDF dr: "<<m_dr<<std::endl;
            if(type =="on"){
                
                m_pAnalysisVariables->SetRDFCalculationActive();
            }
            ...
            std::cerr<<"----> Error: Bad keyword for input.ana: "<<firstword<<std::endl;
            input.close();
            exit(1);
        }
...
//---- position update
            ...
            input>>str>>type;
            if(type == EvolveVerticesByMetropolisAlgorithm::GetDefaultReadName() || type == "MetropolisAlgorithmOpenMP"){  // EvolveVerticesByMetropolisAlgorithm
...
//---- AlexanderMove
                ...
                input>>str>>type;
                if(type == AlexanderMoveByMetropolisAlgorithm::GetDefaultReadName() || type == "MetropolisAlgorithmOpenMP"){  // MetropolisAlgorithm
                    double rate;
                    input >> rate;
...
//---- end //
    //---- inclsuion move
                    ...
                    input>>str>>type;
                    if(type == InclusionPoseUpdateByMetropolisAlgorithm::GetDefaultReadName() || type=="MetropolisAlgorithmOpenMP" ){  // MetropolisAlgorithm
...
// dynamic box
            ...
            if (type == PositionRescaleFrameTensionCoupling::GetDefaultReadName() || type=="IsotropicFrameTensionOpenMP") {
...
//-- State class variable
        ...
        else if(firstword == "INCLUSION")
        {
            ...
            if(firstword.at(0)!=';') {
                std::cout<<"Warning: unrecognized keyword in the input file *** "<<firstword<<" ***, skipping\n";
            }
            getline(input,rest);
        }

        // A keyword line with fewer values than expected (e.g. "FluctuationSpectrum = on 5 5"
        // instead of "= on 5 5 0") leaves the stream in a fail state after the last successful
        // extraction spills into the next line's token. Left unchecked, every line after it in
        // the file is silently skipped. Report it and recover so the rest of the file still loads.
        if (input.fail() && !input.eof()) {
            std::cout<<"Warning: missing or malformed value(s) after keyword '"<<firstword
                      <<"' in the input file - check it has all the required fields; continuing\n";
            input.clear();
        }
    }
...
// We read the folder of TrajTSI files
            ...
            if (no_incs==0){
                ...
                // Count the unique inclusion types
                std::vector<inclusion *>& all_inclusions=m_Mesh.GetInclusion();
                std::vector<int> inclusion_types(no_incs);
                int j=0;
                for (std::vector<inclusion *>::iterator it = all_inclusions.begin() ; it != all_inclusions.end(); ++it){
                    inclusion_types[j]=(*it)->GetInclusionType()->ITid;
                    
                    j++;}

                std::vector<int> unique_types;
                for (int i = 0; i < no_incs; ++i) {
                    bool already_exists = false;
                    for (size_t k = 0; k < unique_types.size(); ++k) {
                        if (inclusion_types[i] == unique_types[k]) {
                            already_exists = true;
                            break;
                        }
                    }
                    if (!already_exists) {
                        unique_types.push_back(inclusion_types[i]);
                    }
                }
                
                int num_unique_types = unique_types.size();
                std::cout << "Number of unique inclusion types: " << num_unique_types << std::endl;
                m_pAnalysisVariables->SetNumberInclusionTypes(num_unique_types);
            }
...
//Update initial step and final step
        ...
        if (m_RestartAnalysis==false){
            ...
            if (m_pAnalysisVariables ->GetRDFCalculationActive()==true){
                        std::cout<<"RDF calculation is active"<<m_dr<<std::endl;
                        m_pRDF = new RDF(this,m_dr);
                        m_pRDF->OpenOutputStreams(true);
            }
            ...
            if (m_pAnalysisVariables ->GetRDFCalculationActive()==true){
                        m_pRDF->OpenOutputStreams(false);
            }
...
//---> to update each vertex and edge energy. Up to now May 2024, edge energy is not zero when both vertices has inclusions
    ...
    std::cout<<" Total Energy: "<<m_pEnergyCalculator->GetEnergy()<<std::endl;
