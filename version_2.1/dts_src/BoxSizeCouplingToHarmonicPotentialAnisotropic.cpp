...
=================================================================================================================
*/
BoxSizeCouplingToHarmonicPotentialAnisotropic::BoxSizeCouplingToHarmonicPotentialAnisotropic(int period, double kx, double ky, double kz, double a0x, double a0y, double a0z, std::string direction, State *pState)  :
        m_pState(pState),
...
BoxSizeCouplingToHarmonicPotentialAnisotropic::~BoxSizeCouplingToHarmonicPotentialAnisotropic() {
    
}
void BoxSizeCouplingToHarmonicPotentialAnisotropic::Initialize() {
    
    std::cout<<"---> the algorithm for box change involves applying: "<< GetDefaultReadName()<<" \n";
    m_pBox = (m_pState->GetMesh())->GetBox();
}
bool BoxSizeCouplingToHarmonicPotentialAnisotropic::ChangeBoxSize(int step){
    /**
...
bool BoxSizeCouplingToHarmonicPotentialAnisotropic::AnAtemptToChangeBox(double lx,double ly, double lz, double temp){
...
//---> new global variables
    ...
    Vec3D newDBox = (*m_pBox)-m_Box_0;
    double boxDE = 0;
    for (int i=0;i<3;i++) {
        boxDE += m_K(i)*(newDBox(i)*newDBox(i) - OldDBox(i)*OldDBox(i));
    }
...
bool BoxSizeCouplingToHarmonicPotentialAnisotropic::VertexMoveIsFine(double lx,double ly, double lz){
    /**
...
bool BoxSizeCouplingToHarmonicPotentialAnisotropic::CheckLinkLength(double lx,double ly, double lz){
    /**
...
double BoxSizeCouplingToHarmonicPotentialAnisotropic::StretchedDistanceSquardBetweenTwoVertices(vertex * v1,vertex * v2, double lx, double ly, double lz){
    /**
...
bool BoxSizeCouplingToHarmonicPotentialAnisotropic::CheckFaceAngleOfOneLink(links* p_edge) {
...
bool BoxSizeCouplingToHarmonicPotentialAnisotropic::CheckFaces() {
...
void BoxSizeCouplingToHarmonicPotentialAnisotropic::SetDirection(const std::string& direction) {
    /**
    ...
    // Reset direction vector to zeros
    m_Direction = {0, 0, 0};

    // Map character directions to indices
    for (char axis : direction) {
        switch (axis) {
            case 'X': m_Direction(0) = 1; break;
            case 'Y': m_Direction(1) = 1; break;
            case 'Z': m_Direction(2) = 1; break;
            default:
                std::cout << "---> Error: direction '" << axis << "' is unknown\n";
                return;
        }
    }
}
std::string BoxSizeCouplingToHarmonicPotentialAnisotropic::CurrentState(){
