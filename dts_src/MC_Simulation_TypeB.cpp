 ...
 */
MC_Simulation_TypeB::MC_Simulation_TypeB(State *pState)
{
...
///=======
//== Read MC_Simulation_TypeB variables
//===========
std::cout<<std::fixed;
...
for (int mcstep=ini;mcstep<final+1;mcstep++)
{
    ...
    for (int j=0;j<HalfLinkno;j++)
    {
       // int m=Random1.IntRNG(HalfLinkno);
        links *Tlinks = m_pHalfLinks1.at(j);  // chose a link randomly
    ...
    for (int j=0;j<m_pInclusions.size();j++)
    {
           // int n=Random1.IntRNG(m_pInclusions.size());
            inclusion *linclusion = m_pInclusions.at(j);   //
            mc_IncMove->MC_Move_AnInclusion(linclusion,&Random1);
            InRate+=mc_IncMove->GetMoveValidity();
            totalinmove++;
    }
    // box move
    if( FrameTensionCouplingFlag==true && mcstep%SigmaPTau == 0)
        {
...
//======================================================================================================
//======================================================================================================
MC_Simulation_TypeB::~MC_Simulation_TypeB()
{
}
//================A function to calculate energy of the mesh from scratch  ==================================
double  MC_Simulation_TypeB::SystemEnergy(State * pState)
{
    double en = 0;
...
bool MC_Simulation_TypeB::CheckMesh(MESH *pMesh)
{
    bool isok = true;
...
double MC_Simulation_TypeB::CheckLengthBetweenTwoVertex(vertex* v1, vertex* v2, Vec3D *pBox)
{
...
double   MC_Simulation_TypeB::CheckFaceAngle(links * l, Vec3D *pBox)
{
    vertex* v1 = l->GetV1();
    vertex* v2 = l->GetV2();
    vertex* v3 = l->GetV3();
...
Vec3D   MC_Simulation_TypeB::CalculateNormal(vertex* v1 ,vertex* v2 ,vertex* v3, Vec3D *pBox)
{
...
//==============================================================================================================
void  MC_Simulation_TypeB::CenterIntheBox()
{
...
void MC_Simulation_TypeB::ReadIndexFile(std::string indexfilename)
{
    Nfunction f;
...
//===============
//=== end of the MC_Simulation_TypeB
//==============================
